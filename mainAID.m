...
% Chen Gong, Dacheng Tao, Stephen J. Maybank, Wei Liu, Guoliang Kang, Jie Yang. Multi-modal curriculum learning
% for semi-supervised image classification. IEEE Transactions on Image Processing (TIP), 25 (7): 3249-3260, 2016.
...
load('AID\GroundTruth.mat');
ClassTotal = max(GroundTruth);

load AID\DatasetSplitIdx
% %===AID===%
% % Load View 1 (densenet) Features
 load ('AID\densenet201_AID_fc.mat'); F1=sparse(double(denseFeatures));
% % Load View 2 (inceptionresnetv) Features
 load ('AID\inceptionresnetv2_AID_fc.mat'); F2=sparse(double(denseFeatures));
% % Load View 3 (inceptionresnetv) Features
 load ('AID\resnet101_AID_fc.mat'); F3=sparse(double(denseFeatures));
